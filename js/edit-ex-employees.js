...
$(document).ready(function() {
	exEmployeeID = parseInt($("#ex-employee-id").val().trim());
	...
	getEmployees();
	let fd = new FormData();
	fd.append("id", exEmployeeID);
	fetch(PHP_URL+"/employer/get_ex_employee_by_id", {
		method: 'POST',
		body: fd
	})
		.then(response => response.text())
		.then(async (response) => {
			exEmployeeAllowances = JSON.parse(response);
			$("#months").prop('selectedIndex', parseInt(exEmployeeAllowances['month']));
			let year = parseInt(exEmployeeAllowances['year']);
			for (let i=0; i<years.length; i++) {
				if (years[i] == year) {
					$("#years").prop('selectedIndex', i);
					break;
				}
			}
			$("#taxable").prop('checked', parseInt(exEmployeeAllowances['taxable'])==1);
			$("#description").val(exEmployeeAllowances['description']);
			let employees = exEmployeeAllowances['employees'];
			selectedEmployees = employees;
			let allowances = exEmployeeAllowances['ex_employee_allowances'];
			for (let i=0; i<employees.length; i++) {
				let employee = employees[i];
				$("#selected-employees").append("<tr>" +
					...
					"                                        <td><input name=\"amount\" id=\"amount-" + employee['id'] + "\" placeholder=\"Enter amount\" type=\"number\" class=\"form-control\" value='"+allowances[i]['amount']+"' onchange='checkAmount(" + i + ", "+employee['id']+")'></td>" +
					"                                        <td><button onclick='delete(" + i + ")' class='btn-shadow p-1 btn btn-danger btn-sm show-toastr-example'>Delete</button></td>" +
					"                                    </tr>");
			}
		});
...
function save() {
	...
	fd.append("month", $("#months").prop('selectedIndex'));
	...
	fetch(PHP_URL+"/employer/update_ex_employee", {
		method: 'POST',
		body: fd
	})
		.then(response => response.text())
...
function cancel() {
	if (confirm("Are you sure you want to trash away your editing data?")) {
		window.history.back();
	}
}
