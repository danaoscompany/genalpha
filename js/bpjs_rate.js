	...
	fetch(PHP_URL+"/employer/get_employees", {
		...
		.then(async (response) => {
			...
			for (let i=0; i<rates.length; i++) {
				let rate = rates[i];
				let salary = parseInt(rate['job']['salary']);
				let jkk = 0.54*salary/100;
				let jkm = 0.3*salary/100;
				$("#rates").append("<tr>" +
					"                                        <th scope=\"row\">"+(i+1)+"</th>" +
					"                                        <td>"+rate['id']+"</td>" +
					"                                        <td>"+rate['user']['first_name']+" "+rate['user']['last_name']+"</td>" +
					"                                        <td>"+rate['job']['position']+"</td>" +
					"                                        <td>Active</td>" +
					"                                        <td>"+jkk+"</td>" +
					"                                        <td>"+jkm+"</td>" +
					"                                    </tr>");
...
function downloadXLS() {
	...
	var ws_data = [];
	ws_data.push([
		"Employee ID", "Full Name", "Organization", "Status", "JKK Deduction", "JKM Deduction"
	]);
	for (let i=0; i<rates.length; i++) {
		let rate = rates[i];
		let salary = parseInt(rate['job']['salary']);
	...
	fetch(PHP_URL+"/employer/get_employer_by_id", {
		...
		.then(response => response.text())
		.then(async (response) => {
			let employerInfo = JSON.parse(response);
